...
| 右上メニュー | オーバーレイ | — | `[遷移]` 閉じる、プライバシーポリシー、ログアウト確認 |
| プライバシーポリシー | 画面 | — | `[表示]` ポリシー本文、`[遷移]` 前画面へ戻る |
| ログアウト確認 | オーバーレイ | — | `[表示]` 確認メッセージ、`[実行]` ログアウト確定、`[遷移]` キャンセル |
| ログイン画面 | 画面 | コード送信前／送信後 | `[入力]` メールアドレス、コード、`[表示]` 送信先メールアドレス、共通のログイン失敗表示、`[実行]` コード送信、コード再送、ログイン、`[遷移]` プライバシーポリシー |
| 記録画面 | 画面 | 開始前 | `[実行]` 睡眠開始ボタン、`[遷移]` 追加 |
| 記録画面 | 画面 | 睡眠中 | `[表示]` 睡眠中の状態、落ち着いた背景、操作案内、`[実行]` ロールカーテンを上げる起床記録 |
| 記録画面 | 画面 | 起床直後 | `[表示]` 明るい完了表示 |
| グラフ画面 | 画面 | — | `[表示]` 表示中の年月、グラフ本体、`[実行]` 初期表示へ戻る、`[遷移]` 日付選択、睡眠一覧 |
| 日付選択 | オーバーレイ | — | `[入力]` 日付、`[実行]` 日付を選択、`[遷移]` キャンセル |
| 睡眠一覧 | オーバーレイ | — | `[表示]` 表示期間、睡眠記録一覧、末尾の「新規追加」、`[遷移]` 閉じる、新規追加、編集、削除確認 |
| 追加・編集入力 | オーバーレイ | 追加／編集 | `[入力]` 開始日時、終了日時、`[実行]` 保存、`[遷移]` キャンセル |
| 削除確認 | オーバーレイ | — | `[表示]` 確認メッセージ、`[実行]` 削除確定、`[遷移]` キャンセル |
